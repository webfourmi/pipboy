...
////////////////////////////////////////
    ...
    function pad2(n){ return String(n).padStart(2,"0"); }
      ...
      // Date -> "YYYY-MM-DDTHH:MM" (format datetime-local)
      function toDateTimeLocal(ts){
        const d = new Date(ts);
        return `${d.getFullYear()}-${pad2(d.getMonth()+1)}-${pad2(d.getDate())}T${pad2(d.getHours())}:${pad2(d.getMinutes())}`;
      }
      
      // "YYYY-MM-DDTHH:MM" -> timestamp (ms)
      function fromDateTimeLocal(str){
        // new Date("YYYY-MM-DDTHH:MM") marche en local sur la plupart des navigateurs modernes
    ...
    const openArchivesBtn = $("openArchives");
    const archivesModal = $("archivesModal");
    const closeArchivesBtn = $("closeArchives");
    const closeArchivesBackdrop = $("closeArchivesBackdrop");
    
    function openArchives(){
      if (!archivesModal) return;
      archivesModal.classList.add("show");
      archivesModal.setAttribute("aria-hidden","false");
      renderArchives();
    }
    function closeArchives(){
      if (!archivesModal) return;
      archivesModal.classList.remove("show");
      archivesModal.setAttribute("aria-hidden","true");
    }
    
    if (openArchivesBtn) openArchivesBtn.addEventListener("click", openArchives);
    if (closeArchivesBtn) closeArchivesBtn.addEventListener("click", closeArchives);
    if (closeArchivesBackdrop) closeArchivesBackdrop.addEventListener("click", closeArchives);
    ...
    if (archivesList){
      archivesList.addEventListener("click", async (e) => {
        ...
        if (act === "copy"){
          try { await navigator.clipboard.writeText(item.text || ""); alert("Copié ✅"); }
    ...
    // Copier tout / vider
    const exportArchivesBtn = $("exportArchives");
    if (exportArchivesBtn) exportArchivesBtn.addEventListener("click", async () => {
      const { entries } = getJournalData();
      const sorted = entries.slice().sort((a,b)=> (b.ts||0) - (a.ts||0));
      const text = sorted.map(e => `[${formatTs(e.ts)}]\n${e.text}\n`).join("\n");
      try { await navigator.clipboard.writeText(text); alert("Archives copiées ✅"); }
      catch { alert("Copie impossible"); }
    });
    
    const clearArchivesBtn = $("clearArchives");
    if (clearArchivesBtn) clearArchivesBtn.addEventListener("click", () => {
      const ok = confirm("Vider toutes les archives de ce profil ?");
      if (!ok) return;
      setJournalData(getJournalData().draft, []);
      renderArchives();
    });
    ...
    function normalizeProfileData(d){
      ...
      out.logDraft = (typeof out.logDraft === "string") ? out.logDraft : (typeof out.log === "string" ? out.log : "");
    ...
    function toggleSheetLock(){
      ...
      data.sheet = data.sheet || defaultProfileData().sheet;
      data.sheet.locked = !data.sheet.locked;

      setProfileData(id, data);
      applySheetLockUI(data.sheet.locked);
    }
    // ✅ Listener bouton lock
    const lockBtn = $("toggleSheetLock");
    if (lockBtn) lockBtn.addEventListener("click", toggleSheetLock);

    // ===== FICHE PJ =====
    const SKILLS = [
      "adaptation","astrophysique_mathematiques","biologie_geologie",
      "bricoler_bidouiller","comprendre_etrange","convaincre",
    ...
    function loadSheetToUI(data){
      ...
      applySheetLockUI(!!(data.sheet && data.sheet.locked));
    }
    ...
    function readSheetFromUI(){
      ...
      sh.locked = !!(data.sheet && data.sheet.locked);
    ...
    function saveSheet(){
      ...
      if (current.sheet && current.sheet.locked){
    ...
    // Boutons journal (DATA)
    const btnArchive = $("saveLog");
    const btnClearDraft = $("clearLog");
    ...
    if (btnArchive) btnArchive.addEventListener("click", () => {
      ...
      // on archive + on vide la note active
    ...
    if (btnClearDraft) btnClearDraft.addEventListener("click", () => {
      setJournalData("", getJournalData().entries);
      if ($("log")) $("log").value = "";
      if ($("logWhen")) $("logWhen").value = toDateTimeLocal(Date.now());
    });
    
    // Charger la note active au changement de profil
    // Dans loadActiveProfileToUI(), remplace l’ancien set du log par:
    (function(){
      // tu peux mettre ça directement dans loadActiveProfileToUI()
    })();
    ...
    function applyImport(){
      ...
      if (obj.sheet || obj.log || obj.inv || obj.quests) {
        ...
        if (typeof obj.log === "string") data.logDraft = obj.log;
    ...
    function applyImportActiveProfile(){
      ...
      if (obj && typeof obj === "object" && obj.data && typeof obj.data === "object") {
        ...
        data = {
          logDraft: (typeof obj.log === "string") ? obj.log : "",
      ...
      if (!data) { alert("Format non reconnu 😵‍💫"); return; }
    ...
    function applyPackActive(){
      ...
      catch { alert("Pack MJ: JSON invalide 😵‍💫"); return; }
